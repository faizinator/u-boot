...
static int cpsw_eth_probe(struct udevice *dev)
{
	...
	ti_cm_get_macid(dev, pdata->enetaddr);
...
static int cpsw_eth_ofdata_to_platdata(struct udevice *dev)
{
	...
	data = malloc(sizeof(struct cpsw_platform_data));
	...
	fdt_for_each_subnode(subnode, fdt, node) {
		...
		if (!strncmp(name, "cpsw-phy-sel", 12)) {
			data->gmii_sel = cpsw_get_addr_by_node(fdt,
								    subnode);
			...
			data->phy_sel_compat = fdt_getprop(fdt, subnode, "compatible",
						     NULL);
	...
	ret = ti_cm_get_macid_addr(dev, active_slave, data->syscon_addr);
	if (ret < 0) {
